...
class ProjectedMaterialView:
    ...
    @classmethod
    def from_blender_image(cls, *, name: str, image: bpy.types.Image, azimuth_degrees: float, elevation_degrees: float, flip_x: bool=False, weight: float=1.0, mask: BinaryMask | None=None) -> 'ProjectedMaterialView':
        ...
        return cls(name=str(name), image=image_buffer, transform=compile_projection(azimuth_degrees=azimuth_degrees, elevation_degrees=elevation_degrees, flip_x=flip_x), mask=mask, camera_direction=direction_to_camera(azimuth_degrees=azimuth_degrees, elevation_degrees=elevation_degrees), weight=normalized_weight)
